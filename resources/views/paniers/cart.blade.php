...
<div class="container mt-5 mb-5">
    ...
    <a href="{{ route('boutiques.boutique') }}" class="btn btn-primary col-md-3 mx-auto d-block text-info">
    ...
    <form action="{{ route('paniers.updateCart') }}" method="POST">
        ...
        <table class="table table-bordered table-striped">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        {{ number_format($item->boutique->prixpromo > 0 ? $item->boutique->prixpromo : $item->boutique->prix, 2) }} F CFA
                    </td>
                    <td>
                        <input type="number" name="quantity[{{ $item->id }}]" value="{{ $item->quantity }}" min="1" class="form-control w-25 d-inline">
                    </td>
                    <td>
                        {{ number_format(($item->boutique->prixpromo > 0 ? $item->boutique->prixpromo : $item->boutique->prix) * $item->quantity, 2) }} F CFA
                    </td>
                    <td>
        ...
        <div class="d-grid gap-2">
            ...
            <a href="{{ route('boutiques.boutique') }}" class="btn btn-secondary col-md-3 mx-auto">
                Continuer les achats
            </a>
            <a href="{{ route('paniers.validation.page') }}" class="btn btn-success col-md-3 mx-auto">
